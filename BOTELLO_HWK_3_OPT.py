...
Estimate the lowest cost mix of wind, solar, and energy storage required to run the entirety of ERCOT in 2022.
"""
...
ESS_p_var_cost          = 5000      # ESS discharge cost $/GW

curtailment_cost        = 1000      # curtailment penalty $/GW


# create the model
model = AbstractModel(name = 'BOTELLO_HWK_3_OPT')   #solar-wind-storage model, model name not matching file name seems cause an error. 

# create model sets. Sets are components used to index other components. 
...
#Create parameters. Set up paramets where data will be stored.
model.solar             = Param(model.t)
model.wind              = Param(model.t)
model.demand            = Param(model.t)  
model.costs             = Param(model.tech, initialize={'s_cap' : solar_cap_cost,
                                                        'w_cap' : wind_cap_cost,
                                                        'ESS_power_cap' : ESS_p_cap_cost,
                                                        'ESS_energy_cap' : ESS_e_cap_cost})

## load data into parameters. Solar and wind data are houlry capacity factor data; demand data is also hourly data in GW. 
data = DataPortal()
...
# solve the model
opt = SolverFactory('glpk')   #Can also use gurobi solver (paid solver)
...
#Use the following to run the model in the terminal:
